...
## Soul-Driven Convergence Is Legitimate — Don't Respawn for Manufactured Difference (2026-09-23)
**Learning**: When the product's soul dictates a colour law (e.g. colour reserved for STATE, no gain/loss euphoria, opaque matter), independent council apprentices will converge on the same palette family (warm ground + one reserved signal). That is convergence on the essence, not on a generic default. The anti-convergence audit should pass it and flag only default-driven rhyme; respawning pushes apprentices away from the soul toward difference for its own sake.
**Apply when**: A design council's candidates converge — check whether the shared trait traces to the Soul Brief before treating it as slop.

## Data-Dense Products Can Reject the Minimal/Atmospheric Tiers as Too Airy (2026-09-23)
**Learning**: For dashboards a user opens all day (trading, monitoring, ops), the council's instinct toward negative space and a big airy hero under-serves. "Density-with-discipline" is a distinct substance posture: dense professional data tables, calm achieved through hierarchy (size, weight, tabular alignment, one signal colour) rather than emptiness — not ticker-wall maximalism. Ground the density in domain research (how expert tools lay out the same data) rather than inventing it.
**Apply when**: Forging a Touchstone for an all-day, data-heavy tool.

## Theme Governs the Calm State; the Alarm State Is Theme-Independent (2026-09-23)
**Learning**: For any product with a calm→alarm transition, the light/dark theme governs only the calm surface; the alarm state floods to ONE fixed critical palette in every theme. An alarm doesn't care about the viewer's colour preference, and a single alarm identity strengthens the "becomes a different object" signature.
**Apply when**: Designing theming for a product with a critical/alarm state.
