...
## SKILL.md Frontmatter Is Two Layers — Portable Spec vs Harness Extensions (2026-03-26, corrected 2026-08-15)
**Learning**: SKILL.md frontmatter splits into a portable layer and a harness layer, and the split — not any fixed field list — is what to reason about. The **Agent Skills spec** allows `name`, `description`, `license`, `compatibility`, `metadata`, `allowed-tools`; a spec-strict validator rejects anything else. **Harness extensions** add fields that a spec validator will reject — in Claude Code these include `argument-hint`, `disable-model-invocation`, `user-invocable`, `model`, `context`, and others. Consequence for tool-neutral skill repos: keep the canonical source at the spec layer and inject harness-specific fields at deploy time, never author them into the neutral copy. Field lists churn — re-read the harness's frontmatter reference rather than trusting a cached list (this entry itself asserted `allowed-tools` and `context` were invalid; both are documented fields).
**Apply when**: Authoring or reviewing SKILL.md files, and whenever a build step injects or strips frontmatter across tool targets.
