...
## Codegen Must Escape EVERY Interpolated String Structurally, Not Spot-Fix the Delimiter That Broke (2026-07-19)
**Learning**: When a script emits source or data files (codegen, fixture manifests, config, SQL, CSV), serialize every interpolated string value through the target format's canonical serializer — the language's own string serializer (e.g. `JSON.stringify` for JS/TS/JSON), parameterized queries for SQL, a real CSV writer for CSV — rather than wrapping values in quotes by hand. Fixing only the one value that happened to break this run (an apostrophe in a name, a quote in a string) leaves every OTHER value one odd character away from the identical bug: the next name with an apostrophe silently produces a syntax error, a broken row, or an injection. Patching the single failing case is symptom-fixing; the class fix is uniform structural escaping. This is review-catchable precisely because the generating author tends to reach for the narrow fix under momentum.
**Apply when**: Reviewing any codegen / templating / data-export / query-building path. Smell: a hand-rolled quote-wrap with a manual `.replace()` on one field while sibling fields interpolate raw — flag it and require the canonical serializer applied to all.

## Test Fixtures Must Match the Product's Real Domain/Locale, Not the Most Convenient Public Dataset (2026-07-19)
**Learning**: When sourcing test data, grabbing the first readily-downloadable public corpus optimizes for "get a test running" over "does this represent the actual user?" Off-domain fixtures — wrong locale, currency, language, date format, units, vernacular — validate the WRONG thing: the suite can pass green while real user inputs fail, because the system was never exercised on the inputs it will actually see (a product serving one region's conventions tested only against a foreign dataset with different currency, tax, and formats). Match fixtures to the product's real-world domain even when that means GENERATING them (with exact authored ground truth) instead of downloading a ready-made off-domain set. Keep a convenient real-world set only where its value is domain-independent (e.g. messy-scan robustness), and label it as such.
**Apply when**: Reviewing test design, eval/benchmark corpora, or demo/seed data. Ask "does this test data look like what THIS product's real users actually produce?" — green tests over off-domain fixtures are a silent coverage gap.
