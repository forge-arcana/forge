...
**Apply when**: Selecting a TTS provider for any voice-enabled project — model monthly character volume against the ceiling first (see "Free Tier Limits Change Without Notice" for the verification rule).

## Serverless DB + Container Cold Start Warming Strategy (2026-03-29)
**Learning**: Scale-to-zero serverless databases (300-500ms cold start) combined with scale-to-zero compute containers (2-5s cold start at min-instances=0) produce first-request latencies of 4-9 seconds. A health-check cron every 4 minutes during business hours is the free-tier warming strategy — but a warming cron IS an always-on consumer: it converts the free tier's compute-hour allowance into a ~300h/month burn and removes the sleeping that made serverless cheap. Reach for it only when cold-start latency is the binding constraint AND the tier is metered on requests rather than compute hours; otherwise the warming requirement is the signal to move off scale-to-zero entirely.
**Apply when**: deploying any app on serverless DB + serverless compute free tiers where scale-to-zero is enabled.
**Evolved by**: "Serverless-Postgres-by-Default Inverts for Always-On Workloads" (2026-07-19) — if you need a warmer, you no longer have a bursty workload, and a small flat-rate instance wins.
...
**Apply when**: Reviewing any system that must perform an external side effect exactly once per logical item (payments, emails, posts, webhooks, provisioning). Check the uniqueness key for the tenant dimension and the ledger write ordering first. Companion entries: "Anti-Double-Post: Prefer Defer Over Republish When Landing Is Unconfirmable" (smith-learnings.md), "A Transport Error Is Not a Publish Failure — Verify Against the Account" (pry-learnings.md), and the burst-throttled media API entry (global-patterns.md).
