...
**Learning**: When multiple projects share the same Android SDK on a development machine, AVDs with default names (Pixel_7_API_35, Medium_Phone_API_36) collide and create confusion. Prefix every AVD with the project name: single-app projects use `<project>` (e.g., `myapp`), multi-app projects use `<project>-<role>` (e.g., `myapp-admin`, `myapp-client`). A reusable `emulator.js` script pattern (list/nuke/create/launch) manages AVDs from WSL via `powershell.exe` calling Windows-side SDK tools (`ANDROID_HOME`). The nuke command must also clean orphaned `.avd` data directories under `ANDROID_AVD_HOME` — these are left behind when emulators are running during `avdmanager delete`. Each project owns its own script with its own AVD config; no shared emulator state.
**Apply when**: Setting up Android emulator workflows for any Capacitor/hybrid project, especially in multi-project dev environments.
