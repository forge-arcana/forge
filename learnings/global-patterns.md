...
## Gemini MCP for Asset Generation (2026-04-02)
**Learning**: Claude Code can generate images (icons, logos, UI assets) by connecting to Google Gemini via the **nanobanana-mcp** server (`@ycse/nanobanana-mcp`). Setup: (1) user gets a free API key from aistudio.google.com/apikey — no credit card needed for free tier, Pro subscription unlocks image generation, (2) register the MCP server: `claude mcp add nanobanana -s user -- npx -y @ycse/nanobanana-mcp`, (3) pass the API key via env var `GEMINI_API_KEY` in the mcpServers config (never in CLI args — visible in shell history). Free tier: text-only models. Image generation requires paid tier or Pro subscription. Session restart required after adding MCP server.
**Apply when**: A project needs icons, logos, illustrations, or UI mockups generated. Ask the user for their Gemini API key, configure nanobanana-mcp, restart, and generate.
