...
## Non-Production Deployments Must Block Bots and Crawlers (2026-03-29)
**Learning**: Every non-production deployment (staging, preview, dev) must block search engine crawling, AI scrapers, and security scanners. Without protection, staging URLs get indexed by Google, scraped by AI crawlers, and probed by vulnerability scanners — leaking unfinished features and consuming resources. Layer these defenses: (1) `robots.txt` with `Disallow: /` and `X-Robots-Tag: noindex, nofollow` response header, (2) IAM-gated access on Cloud Run (`--no-allow-unauthenticated` + IAP or service account), (3) basic auth or IP allowlisting as fallback for simpler setups, (4) `noindex` meta tag in the HTML `<head>` as a belt-and-suspenders measure. On GCP Cloud Run, the strongest approach is `--no-allow-unauthenticated` which blocks all traffic unless the caller has `roles/run.invoker` — bots never get past the front door. For other hosting (Vercel, Netlify, AWS), use platform-specific access controls (Vercel Password Protection, AWS WAF IP restrictions, Netlify Identity). This must be part of the deployment pipeline, not an afterthought — every `deploy.yml` or CI script that targets a non-production environment should include bot protection as a required step.
**Apply when**: Deploying any application to staging, preview, or dev environments on any hosting platform.
