...
## One Interface Face, a Real-Product Hero, and Sections That Change Shape (2026-09-05)
**Learning**: A cluster of identity/landing rules that recur across consumer-app UI work. (1) One interface face — a display serif on headings over a sans body splits the product into two voices; build hierarchy through weight and tracking on a single family, and let the display face survive only as the logotype beside the brand mark. (2) Check the shared stylesheet's base rules before auditing components: two base rules (a heading selector and a figures utility class) can feed a second face to every heading and every number, so a fix there covers surfaces no class-name grep would list. (3) A marketing page must show the product — illustration plus copy about the product is the tell of an SEO-first page; a static mock of a real screen in the hero fixes it with no new assets. (4) A single centred column at one max width stretched to desktop is the "college project" tell; sections must change shape, and mobile-first means designed to stack, not merely fitting. (5) Never invent third-party brand marks — use published geometry, and put a multicolour mark on a white chip when it sits on a coloured button. (6) When introducing a new primary option, do not silently demote existing ones to muted outlines; keep equal weight absent a stated reason. (7) Inline SVG marks inside flex buttons need `flex-shrink: 0` or narrow buttons collapse them to nothing.
**Apply when**: Auditing or building a consumer app's visual identity and its marketing/landing surface — reach for these before writing per-component overrides, and verify each at narrow (~375px) and wide (~1440px) widths in both themes.
