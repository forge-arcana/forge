...
- **Last session (2026-08-15)**: **Token-triage upgrade** — four changes to the model-tier stack. (1) `/burn` now proves tier binding: `burn-status.sh` aggregates per model within each session (Model-mix column + By-Model totals with % of cost); pricing refreshed to current Claude 5 list rates (old opus row was 3× overstated; fable tier added as the conservative fallback). (2) `protocol.md` gained an optional per-spawn effort qualifier ("low-effort sonnet-tier") with defer-don't-downgrade parity for gates. (3) **Complexity triage**: protocol T1/T2/T3 grade vocabulary; smith grades every heat (Grade column in the build plan, user-overridable) driving build tier (T1 → low-effort sonnet, T2 → sonnet, T3 → opus/inline) and gate depth (T1 verify-only with review batched to the next phase gate — never trimmed; T3 full pass + security always); poke/press/preen collapse dimension fan-out on scoped targets while security legs and opus verdict gates stay full-width. (4) Script-tier extraction closed three deferred items: `plot-scan.sh` (Python-aware topology evidence), `forge-plan.sh` (PLAN-table skeleton from forge-status output; `/forge` hint raised sonnet → inherit, precondition met), `wedge-preview-assemble.sh` + `wedge-parity-check.sh` (tabbed preview via isolated iframes; MD↔HTML token-value check). Scripts built by sonnet-tier subagents and reviewed at top level — the doctrine applied to its own maintenance. Session block for 2026-08-09 moved to memory/recent-history.md. Same-day `/forge` cycle (forge-plan.sh's first production run): cast 8 updated skills + recent-history.md to membrane, baseline → 695607f; absorbed 5 learnings (external-capture chrome-stripping, raw-SQL mapper + inside-the-window tests, collapse-layer test controls → `global-patterns.md` 65→68; shared-view CDP+VNC browser topology → `pry-learnings.md`; overflow-menu signifier → `preen-learnings.md`); 3c review deferred as redundant (full Warden review 2026-08-09, 0 prunes); 3f presentation checks ×2: NO CHANGE. Both purity gates clean. Same-day `/purge` (four-dimension opus fan-out, 43 findings applied): **CRITICAL×2** — a purge log re-leaked the identifiers it recorded scrubbing (second consecutive recurrence; rule now encoded: logs name the leak's CLASS, never the token), and git-tracked `.memory-tracker.json` had published a project filename + a person's name since the 2026-03-19 hash→filename tracker change (2 entries removed by user decision; purity gate's markdown-only file coverage noted as the gap). Stack-guide: 5 pre-2026-07-19 errors fixed that the recorded "full re-verification" missed (Prisma engine-size, Hetzner ~10×→~3-4×, better-auth-capacitor name collision, #8860 mechanism, pgvector >=0.8.2 CVE floor) — that record now carries a partial-warrant annotation. Same-day triage work hardened: smith T3 hint aligned to its own table, protocol gained target-level grading (rule 5), /pound got scoped-collapse (21→selected personas), poke/press/preen collapse paragraphs deduped into their fan-out paragraphs. Learnings: 6 duplicate/conflict consolidations (incl. a probe warming-cron entry that prescribed the anti-pattern its sibling identifies — Evolved-by added), frontmatter learnings corrected (two "invalid" fields are valid; two-layer spec-vs-harness framing), 4 stranded Forge-worthy pry entries promoted (global-patterns 68→69 net of moves). `forge-purge-scan.sh` heading parser made fence-aware (was inventing phantom sections in 9 skills) + 60-line floor on bloat percentages. Memory rosters synced (burn in task list, symlink-retirement topology, /purge double-count).
