...
**During the de-Claude pivot, two parallel layouts exist.** Source-of-truth is `core/`; legacy `skills/` and `scripts/` stay in place until Phase 2 verification.

| Dir | Role | Status |
|-----|------|--------|
| `core/skills/` | Tool-neutral SKILL.md + refs (the source of truth going forward) | Phase 1 — populated |
| `core/scripts/` | Tool-neutral runtime scripts (`$MEMBRANE`-templated) | Phase 1 — populated |
| `core/rules/` | Tool-neutral HARD RULES (forge-governance + development-discipline) | Phase 1 — populated |
| `core/forge.schema.yaml` | Adapter contract (every adapter must satisfy) | Phase 1 — populated |
| `adapters/claude-code/` | Claude Code adapter — manifest, refs, WA-001 scripts | Phase 1 — populated |
| `adapters/bob/` | Bob adapter manifest — Phase 3 acceptance test target | Stub |
| `bin/forge-build` | Build script — assembles `build/<adapter>/` from core + adapter | Phase 1 — wired for claude-code |
| `build/` | Generated deployable output (gitignored) | regenerated by build |
| `skills/` | **Legacy** — current Claude-deployed source. Untouched during transition. Phase 2 deletes after build verification. | legacy, intact |
| `scripts/` | **Legacy** — current scripts. Untouched during transition. Phase 2 deletes after build verification. | legacy, intact |
| `learnings/` | Absorbed team wisdom (populated by `/forge`) | unchanged |
| `memory/` | Team identity & shared conventions (populated by `/forge`) | unchanged |
| `presentation/` | Canonical human-readable overview (`index.html`) | unchanged |
| `.claude/skills/forge/` | Thin bootstrap so `/forge` is discoverable on fresh clone | unchanged |
| `.claude/skills/purge/` | Sole location for `/purge` (forge-internal). Stays Claude-coupled by design — never deployed to user membranes; never migrated to core. | unchanged |

**Build flow**: `bin/forge-build claude-code` reads `core/` + `adapters/claude-code/manifest.yaml` and emits `build/claude-code/{skills,scripts,refs}/` plus a composed `CLAUDE-rules-snippet.md`. The build is functionally equivalent to current `~/.claude/skills/` except for the catalogued intentional improvements (graceful-degradation fallbacks, harness-aware prompt language, WA-001 conditional, vsix Co-Authored-By removal, dig frontmatter add).
...
## Current Context
- **Branch**: main, ahead of origin/main by 4 commits (de-Claude pivot Phase 1, not pushed)
...
- **Phase 1 (structurally complete)**: 22 skills migrated to `core/skills/` (SKILL.md + refs); 9 tool-neutral scripts to `core/scripts/` with `${FORGE_MEMBRANE}` templating; 6 Claude-only WA-001 scripts isolated to `adapters/claude-code/scripts/`; 2 Claude-specific reference docs to `adapters/claude-code/refs/`; HARD RULES extracted to `core/rules/`; `bin/forge-build` produces `build/claude-code/` (verified to assemble correctly: 22 skills + 15 scripts + 2 refs + composed rules).
- **Phase 2 (next)**: Diff `build/claude-code/skills/` against current `~/.claude/skills/`, confirm all diffs map to the catalogued intentional improvements, then delete legacy `skills/` and `scripts/` source trees. Add a `forge-build --deploy` mode that copies build output into `~/.claude/`.
- **Phase 3**: Bob adapter live test. Investigate Bob's prompt primitive (currently stubbed as "inline_chat_prompt") and Orchestrator-mode sub-agent semantics. Build `adapters/bob/` properly. Run `forge-build bob` against a real Bob install and verify `/forge` cycle works.
