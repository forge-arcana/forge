...
- **Branch**: `main`. 2 commits ahead of origin (Phase C `48b5289` + today's stale-reference sweep). Both unpushed pending verification.
- **Active work**: Phase C (membrane goes Generic, `~/.agents/` canonical store + symlink shim from `~/.claude/`) verified end-to-end against live state — `cast-deploy.sh --verify` passes for all 23 skills + 3 runtime scripts; `forge-status.sh` reads through the symlink transparently; pre-migration backup at `/tmp/claude-pre-phase-c.bak` (676MB) intact. Today's follow-up sealed a separate drift bug from the `0486e53` `claude-helpers/` rename: 12 stale `core/scripts/agent-preflight.sh` and `core/scripts/install-token-hook.sh` references corrected to `claude-helpers/scripts/...` across 7 files (forge/SKILL.md, forge/protocol.md, smith, temper, wedge, cicd, .claude/skills/forge bootstrap copy). Preflight script verified runnable from new path (exit 0).
