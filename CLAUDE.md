...
- **Branch**: `main`. Pushed through `a1f366b`. (This line is itself a mirror of push state — it claimed the 2026-07-19 stack-guide commit was pending long after it had shipped. Prefer `git log origin/main..HEAD` over trusting it.)
- **Active work**: No active workarounds. Generic Forge v2 Outstanding items tracked below. Model-remap follow-ups mostly closed 2026-07-17; remaining: forge PLAN-mechanics scripts, wedge preview-assemble + parity check.
- **Last session (2026-07-26)**: **Provenance for mirrored state** — absorbed a cross-cutting pattern into `global-patterns.md`: a repo value describing something external (deployed version, vendor hardware, price, or "what artifacts exist") is a MIRROR, and with no record of when it was last confirmed a stale one is indistinguishable from a correct one. Compounding failure: a test asserting a mirror's literal CONTENT turns staleness into green CI. Mitigations encoded — provenance entries + a staleness test that fails loudly, assert shape not content, a citation is a promise until fetched, and find the producer's write path before trusting a population (0%/100% bucket is the tell). Also fixed `forge-status.sh`: `console.log(<number>)` is ANSI-coloured by both node and bun when colour is forced, so the codes reached `[[ -eq ]]` and aborted every skill-learning comparison with "operand expected" — the new-entry counts had been silently dropping. Now `process.stdout.write(String(n))`.
- **Prior session (2026-07-19)**: **Stack-guide agentic-era pivot** — GCP demoted from sole hosting/data default to re-justified peer, via a 16-agent web-grounded research pass. `stack-guide.md`: container-first Cloud Run/CF Containers co-defaults, Neon default prod DB (Cloud SQL/PlanetScale escalations), R2 storage, Infisical secrets, OTLP seam, new Hosting Decision Framework + DR/Residency sections. `probe`+`prime` deviation rules re-pointed (framework layer pinned; hosting/DB selected by scale/needs signals — anti-trend, not anti-movement); `cicd`/`press`/`plot` cascades. All 25 encoded claims re-verified against live sources post-write; 5 errors corrected. Saddle held out of the guide until its GCP/CF brokers ship.
