...
- **2026-09-05 → 2026-09-23**: one-face typography + product-led landing defaults (preen Dim 7/8, smith visual-identity gate, "Visual Decisions Are Shown, Then Built" HARD RULE); 2026-09-23 fold + curation, with four deferred PROMOTED candidates listed there. Full detail: `memory/recent-history.md`.
- **2026-09-23 context hygiene**: `cast-deploy.sh --settings` (ridden by `--all`) applies SET-IF-ABSENT settings defaults — currently `autoCompactWindow: 250000`; new "Context Hygiene — A Session Is a Task" HARD RULE. Haiku 4.5 + Opus 5.5 advisor under evaluation (manual pilot, no default).

### Outstanding — Generic Forge v2 follow-ups

- **Next `/purge`**: resume `global-patterns.md` consolidation (76 entries after the 2026-09-23 curation, still above threshold) with the deferred candidates: AGENTS.md trio, auth-territory family, five short-shelf-life stack-specific entries, product-strategy family.
