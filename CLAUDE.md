...
- **Recent**: Smith council review — addressed 7 findings from Karpathy assessment + forge council response. Added verify step (CRITICAL), bounded convergence loop with 5-cycle cap + stall detection (CRITICAL), checkpoint rollback with ledger snapshots, extracted sub-files (apprentice-system.md, art-selection.md, ledger-schema.md), structured decision fields, convergence cost signal, apprentice caps + timeouts. Smith SKILL.md 407→337 lines. Identity doc enriched with full art prose descriptions.
- **Pending**: /cast needed (skills + learnings ahead of membrane), /fold needed (14 user learnings + memory updates)
