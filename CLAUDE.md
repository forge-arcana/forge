...
- **Recent**: 2026-06-21 — Forge cycle (from a Python serving-infra project). Cast: `forge`+`praise` skills (purge-pass frontmatter/format) + `global-patterns.md` 4 consolidations + 2 memory files to membrane. Folded 3 learnings from an internal admin-dashboard `/probe`+`/poke`: OAuth `state` must be browser-bound not just signed (CSRF), ephemeral fallback signing secrets break silently across workers, repeated auth guards belong in one decorator (→ `probe-`/`poke-learnings.md`). Baseline → `cf2fc2b`. Then **fixed the recurring `/pry` orphaned-`memory/`-dir bug at its root** — `protocol.md` + `pry/SKILL.md` post-flight now name the project-memory store explicitly and forbid resolving `memory/` against the skill's own base dir (`0139a52`); folded the stranded reframe (`7ec1e22`) and a new **deploy-target reframe** (match deploy target to workload class — existing team infra is a first-class deviation signal for non-web-app/GPU-serving workloads). A 2nd cast brought probe/poke/pry learnings back to membrane. Baseline → `7ec1e22`.
- **Recent**: 2026-06-28 — **Open-source contribution infra**: `CONTRIBUTING.md` (fork→fold→PR flow, branch-protection notes), `.github/workflows/fold-purity.yml` (CI re-runs the leak gate on every PR, additions-only), `fold-purity-check.sh --diff <base> <head>` mode (mirrors `--staged` semantics for CI; fixed `FORGE_DIR`→repo-root anchoring bug).
- **Recent**: 2026-06-27 — **New HARD RULE: Concise and Direct Voice** (`core/rules/development-discipline.md`) — answer-first, no performed-honesty filler ("honestly", "be brutally honest", etc.), no hedging, no stacked intensifiers. Countered Opus's flowery-verbosity habit (no such rule ever *told* it to say "honest"; the 18 in-skill "honest" usages are load-bearing domain content, left intact). **Wired rule propagation to membranes**: `cast-deploy.sh` gained `deploy_rules`/`verify_rules` + `--rules`/`--verify-rules`, rides `--all`, folds into `--verify`; Phase 2 cast (both `core/` and `.claude/` SKILL.md copies) now always runs `--rules`, writing a marker-delimited `FORGE-RULES` block from `core/rules/` into the membrane global rules file (idempotent, personal content preserved). A teammate's next `/forge` lands the rule in their membrane automatically. Partially closes the Membrane-side AGENTS.md-split outstanding item.
