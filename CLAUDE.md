...
- **Recent**: Bot protection nuance (internal vs customer-facing staging) absorbed into stack-guide + press + global-patterns. 5 orphan tracker entries resolved (4 absorbed, 1 title-mismatch fixed). Art auto-invocation, `/forge` session toggle, SKILL.md-based deploy detection, purge cleansed orphans.
- **Pending**: `/cast` needed to deploy updated stack-guide + press SKILL.md to membrane
