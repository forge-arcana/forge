...
- **Model-remap follow-ups (2026-07-16)** — the tier remap wired hints + fan-out tiers everywhere; deferred pieces: shared `web-cache.sh` (six skills re-derive cache prose per spawn); forge PLAN-mechanics scripts (precondition for raising `/forge` top-level sonnet → inherit); `smith-checkpoint.sh`/`smith-rollback.sh`; `wedge-preview-assemble.sh` + parity value-check; pound/temper consolidation scripts + pound sibling-persona batching; `burn-status.sh --compare`; verify nested-skill model semantics (`/vsix`→`/wrap`, `/cicd`→`/pry` — does a nested skill run at the caller's pinned model?); decide opus frontmatter pinning for prime/pitch/wedge/smith (blocked on whether skill frontmatter persists across turns).
