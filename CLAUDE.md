...
- **2026-08-17 → 2026-08-30**: model-tier ceiling + delegation doctrine (`protocol.md` Model Tiers rules 5-7), tier work-router hook pair promoted to `core/hooks/` and now wired into `settings.json` by `cast-deploy.sh --hooks`, spawn gate in `tier-guard.sh`, `/preen` Dimension 6 (cross-surface consistency) absorbed from the rejected `/polish` draft. Full detail per entry: `memory/recent-history.md`.
- **2026-09-05 (one face + product-led landing)**: two post-launch findings on a shipped consumer app absorbed as defaults. (1) *One interface face*: the Touchstone scaffold gained `typography.faces` (`ui_face` required; `logotype_face` reaches only the wordmark; `code_face` for code only; `heading_face` permitted only with audience + reason sentences, never on figures) and a `figures` token pinned to `tabular-nums` on the UI face; the "distinctive font pairing" floor in `/wedge` became "one characterful interface face"; the Heat 6 review enforces the contract; `wedge-parity-check.sh` skips the two rationale sentences; the Direction Card asks for UI face / wordmark face / second heading face. (2) `/preen` gained **Dimension 7 (Typographic Voice)** and **Dimension 8 (Landing Shape)**, rubric in `core/skills/preen/surface-checks.md`, mechanical evidence from the new `preen-surface-scan.sh` (two-voice table with file:line, tabular coverage, brand-mark path lengths, sticky-nav and demoted-provider counts; exits 2 on a two-voice hit). `/press` Deployment flags "landing shape unreviewed" when a marketing page has no Dimension 8 report. (3) `/smith` gained a **visual-identity gate** in 2a/2b plus a `visualPicks` ledger field: prototype before, each candidate, phone + desktop, light + dark; founder picks; only then code. (4) New HARD RULE "Visual Decisions Are Shown, Then Built" in `core/rules/development-discipline.md`. Decisions on the plan's open questions: monospace is not a permitted second interface face (code only; timestamps and counts are figures); landing checks live in `/preen`, `/press` only checks the review happened. Source plan: `plans/one-face-and-a-product-led-landing.md`.
