...
- **The Masters** (three — distinct domains, complementary roles):
  ...
  - `/wedge` — Master of Aesthetic. Reads Opus + Vow, distills a prose Soul Brief and derives three product-specific essence-lenses, runs a council of three design-apprentices (one lens each), crystallizes the chosen direction into the Touchstone (HTML vision + MD contract with YAML tokens). Drives ONE direction; no hedging. Substance is lens-conditional — refined-minimal is a legitimate commit.
