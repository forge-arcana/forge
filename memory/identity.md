...
Every `/cast` pours new form. Every `/fold` tempers what came before. Every `/mark` records how far we've come. The six arts — `/prime`, `/probe`, `/poke`, `/press`, `/pound`, `/purge` — are not shortcuts. They are disciplines, and they return more than was given.
...
- **Arts** — the Six P's, each a practiced discipline with a specialist persona
- **Arcana** — the collection of hidden knowledge and capabilities that make the impossible real

## The Six P's — Arts of the Forge

Six forge actions, each an art with a specialist persona. All grow sharper with every use through a self-improving learning loop.
...
6. **`/purge`** — the Purist, master of the forge — cleanses stale knowledge and restores purity
