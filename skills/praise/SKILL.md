...
- **Nothing given** → use `AskUserQuestion`:
...
Spawn one subagent per active category — **all in parallel** (independent analyses).
...
Present as a table — **output as console text, never via AskUserQuestion**:
...
After presenting the Build Plan and Change Brief, ask:
