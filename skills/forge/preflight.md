...
2. If not found, error: "forge-path not found in ~/.claude/CLAUDE.md. Run /forge to configure."
...
**Baseline**: The last-cast commit SHA stored in `~/.claude/.last-cast.json` (written by `/forge` after each cast phase). If no baseline exists (first cycle, or SHA unreachable after force push), falls back to a two-way heuristic (`git diff HEAD origin/main`).
...
2. **Present** a unified PLAN table as console text (markdown — never via AskUserQuestion, which compresses large tables)
3. **Confirm** via `AskUserQuestion` with simple options: "Apply selected" / "Adjust" / "Cancel" (skipped in `--dry`)
