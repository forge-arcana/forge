...
Generate or update `restart.sh` and `kill-zombies.sh` in the **`dev/` directory** (never in `scripts/` — that's for production scripts like build, deploy, release, and CI). Create `dev/` if it doesn't exist.

Also add npm script aliases in `package.json` for ergonomic invocation:
```json
"scripts": {
  "restart": "bash dev/restart.sh",
  "kill": "bash dev/kill-zombies.sh"
}
```
