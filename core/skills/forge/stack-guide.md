...
| **ORM** | Drizzle | No codegen step, no generate-before-typecheck gate (the CI trap Prisma's client generation creates). Note: the old "Prisma ships a 2-3MB Rust engine binary" argument is dead — Prisma 7 (GA 2025-11-19) replaced it with a TS/WASM query compiler (~1.6MB). Pick Drizzle for the SQL-shaped API and zero-codegen build, not for bundle size. SQL-like API, native pgvector support, faster cold starts. Schema-as-code in TypeScript. |
...
| **Database** | PostgreSQL — Neon (default) | Neon for **all environments including production** — serverless, scale-to-zero, copy-on-write branch-per-PR (an agent runs migrations on a temp branch, then promotes — arguably safer than the human-console path), native pgvector (**require >= 0.8.2** — CVE-2026-3172, integer underflow → buffer overflow in parallel HNSW build, affects 0.6.0-0.8.1, lets an authenticated user read across relations or crash the server. Current: 0.8.6.). Cloud SQL and PlanetScale are situational escalations (see Database Strategy + DR/Residency). Drizzle abstracts the provider — only the connection string changes. |
...
| **Day 1 (dogfood)** | Postgres in Docker Compose on the app box | Single box, private bridge, no managed dependency. Mandatory: offsite `pg_dump` + one tested restore drill (Day-1 guardrail 1). |
| **Production (public launch)** | **Neon (default)** | Scale-to-zero, instant branch restore ("dropped a table at 3am" → branch back online while prod stays up), native pgvector, one MCP + one branching model dev→prod. Floor for prod = **Launch plan (7-day PITR)** — Free's 6h window is disqualifying. |
...
| Cost-dominant / always-on / egress-heavy / EU residency | **Hetzner** (via Kamal 2) | ~3-4× compute-per-dollar and near-$0 egress vs hyperscalers (Hetzner repriced 2026-06-15 — CCX ~2.1-2.7×, CPX ~2.4-3.1×, CX/CAX ~1.3-1.4× increases; the pre-2026 "~10×" figure is dead. Egress remains the durable lever, not raw compute. Re-derive before quoting.); EU residency; dedicated + GPU boxes. Kamal 2 (text-only CLI over SSH, auto-TLS) makes an IaaS box genuinely agent-operable. Cost: no scale-to-zero, you own the DB + patching + backups, and its API tokens are **coarse project-scoped bearers (no OIDC / no short-lived)** — the weakest credential model here, so rotate hard and isolate blast radius by per-env projects. |
...
- Reference: two different packages share this name — verify which you want. `@daveyplate/better-auth-capacitor` (scoped; github.com/daveyplate/better-auth-capacitor) is v0.0.1 and dormant since 2025-05. The unscoped `better-auth-capacitor` on npm is productdevbook's, v0.4.0 (2026-07-05) and actively maintained. `npm i better-auth-capacitor` gets the latter.
